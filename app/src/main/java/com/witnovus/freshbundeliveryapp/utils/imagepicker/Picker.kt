package com.witnovus.freshbundeliveryapp.utils.imagepicker
...
import com.witnovus.freshbundeliveryapp.databinding.PickerActivityBinding
import com.witnovus.freshbundeliveryapp.utils.imagepicker.gallery.BottomSheetMediaRecyclerAdapter
import com.witnovus.freshbundeliveryapp.utils.imagepicker.gallery.BottomSheetMediaRecyclerAdapter.Companion.HEADER
import com.witnovus.freshbundeliveryapp.utils.imagepicker.gallery.BottomSheetMediaRecyclerAdapter.Companion.SPAN_COUNT
import com.witnovus.freshbundeliveryapp.utils.imagepicker.gallery.InstantMediaRecyclerAdapter
import com.witnovus.freshbundeliveryapp.utils.imagepicker.gallery.MediaModel
import com.witnovus.freshbundeliveryapp.utils.imagepicker.interfaces.MediaClickInterface
import com.witnovus.freshbundeliveryapp.utils.imagepicker.interfaces.PermissionCallback
import com.witnovus.freshbundeliveryapp.utils.imagepicker.utils.GeneralUtils.getStringDate
import com.witnovus.freshbundeliveryapp.utils.imagepicker.utils.GeneralUtils.manipulateBottomSheetVisibility
import com.witnovus.freshbundeliveryapp.utils.imagepicker.utils.HeaderItemDecoration
import com.witnovus.freshbundeliveryapp.utils.imagepicker.utils.MediaConstants.IMAGE_VIDEO_URI
import com.witnovus.freshbundeliveryapp.utils.imagepicker.utils.MediaConstants.getImageVideoCursor
import com.witnovus.freshbundeliveryapp.utils.imagepicker.utils.PermissionUtils
...
class Picker : AppCompatActivity() {
    ...
    private lateinit var mBinding: PickerActivityBinding
    ...
    private fun initViews(){
        ...
        if(mPickerOptions.onlyVideo){
            mBinding.textViewMessageBottom.visibility = View.VISIBLE
            mBinding.textViewMessageBottom.text = "Please hold on for video"
        }else{
            mBinding.textViewMessageBottom.visibility = View.VISIBLE
            mBinding.textViewMessageBottom.text = "Please tap for photo"
    ...
    private fun getMedia() {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            val cursor: Cursor? = withContext(Dispatchers.IO) {
                getImageVideoCursor(
                    this@Picker,
                    mPickerOptions.excludeVideos,
                    mPickerOptions.onlyVideo
                )
            }
                ...
                for (i in 0 until mPickerOptions.preSelectedMediaList.size) {
                    for (j in 0 until galleryImageList.size) {
                        if (Uri.parse(mPickerOptions.preSelectedMediaList[i]) == galleryImageList[j].mMediaUri) {
                            true.also { galleryImageList[j].isSelected = it }
    ...
    private fun hideTopViews() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            //mBinding.constraintBottomSheetTop.setBackgroundColor(resources.getColor(R.color.white))
            DrawableCompat.setTint(
                mBinding.imageViewBack.drawable,
    ...
    private fun handleBottomSheet() {
        ...
        mBinding.imageViewCheck.setOnClickListener {
            ...
            mBinding.textViewTopSelect.text = "Tap to Select"
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                mBinding.constraintBottomSheetTop.setBackgroundColor(resources.getColor(R.color.color_theme))
                DrawableCompat.setTint(
                    mBinding.imageViewBack.drawable,
                    resources.getColor(R.color.white)
                )
    ...
    fun hideStatusBar() {
        if (Build.VERSION.SDK_INT >= 16) {
            this.getWindow().setFlags(AccessibilityNodeInfoCompat.ACTION_NEXT_HTML_ELEMENT, AccessibilityNodeInfoCompat.ACTION_NEXT_HTML_ELEMENT);
            this.getWindow().getDecorView().setSystemUiVisibility(3328);
        } else {
